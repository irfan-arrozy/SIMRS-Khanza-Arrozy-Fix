...
public class DlgUpdateUser extends javax.swing.JDialog {
    ...
    private boolean penyakit=false,obat_penyakit=false,dokter=false,jadwal_praktek=false,petugas=false,pasien=false,registrasi=false,tindakan_ralan=false,kamar_inap=false,
            ...
            penilaian_awal_medis_ranap_neonatus=false,penilaian_derajat_dehidrasi=false,penilaian_bayi_baru_lahir=false,
            rl1_3_ketersediaan_kamar=false,skrining_diabetes_melitus=false,laporan_tindakan=false,pelaksanaan_informasi_edukasi=false,
            layanan_kedokteran_fisik_rehabilitasi=false,skrining_kesehatan_gigi_mulut_balita=false,skrining_anemia=false,layanan_program_kfr=false,catatan_poli=false;
    ...
    */
    ...
    private void tampil(String user) {  
        ...
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.penilaian_awal_medis_ranap_neonatus,user.penilaian_derajat_dehidrasi,"+
                "user.penilaian_bayi_baru_lahir,user.rl1_3_ketersediaan_kamar,user.skrining_diabetes_melitus,"+
                ...
                "user.layanan_program_kfr,user.catatan_poli from user where user.id_user=AES_ENCRYPT(?,'nur')");
            try {
                ps.setString(1,user);
                rs=ps.executeQuery();
                if(rs.next()){
                    ...
                    penilaian_bayi_baru_lahir=rs.getBoolean("penilaian_bayi_baru_lahir");rl1_3_ketersediaan_kamar=rs.getBoolean("rl1_3_ketersediaan_kamar");skrining_diabetes_melitus=rs.getBoolean("skrining_diabetes_melitus");laporan_tindakan=rs.getBoolean("laporan_tindakan");pelaksanaan_informasi_edukasi=rs.getBoolean("pelaksanaan_informasi_edukasi");layanan_kedokteran_fisik_rehabilitasi=rs.getBoolean("layanan_kedokteran_fisik_rehabilitasi");
                    skrining_kesehatan_gigi_mulut_balita=rs.getBoolean("skrining_kesehatan_gigi_mulut_balita");skrining_anemia=rs.getBoolean("skrining_anemia");layanan_program_kfr=rs.getBoolean("layanan_program_kfr");catatan_poli=rs.getBoolean("catatan_poli");
                    setTampil();
                }       
      ...
      //  }
       ...
       // }
        ...
        if("[M]Catatan Keperawatan Ranap".toLowerCase().contains(TCari.getText().toLowerCase())){
         ...
         if("[M]Catatan Poli".toLowerCase().contains(TCari.getText().toLowerCase())){
            tabMode.addRow(new Object[]{false,"[M]Catatan Poli",catatan_keperawatan_ranap});
        }
    ...
    private void Simpan2() {
        for(i=0;i<tbUser.getRowCount();i++){ 
            ...
            if("[M]Catatan Poli".equals(tbUser.getValueAt(i,1).toString())){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+TKd.getText()+"','nur')","catatan_poli='"+tbUser.getValueAt(i,2).toString()+"'");
            }
